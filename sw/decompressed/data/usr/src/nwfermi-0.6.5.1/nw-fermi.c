/*
 * Nextwindow Fermi touchscreen driver - Version: 0.6.5.1
 *
...
static void bulk_reset(struct delayed_work* work)
{
	struct usb_fermi* dev = container_of(work, struct usb_fermi, bulk_reset_work);
...
static void bulk_free_urb(struct delayed_work* work)
{
	struct usb_fermi* dev = container_of(work, struct usb_fermi, bulk_free_urb_work);
...
#else
		usb_buffer_free(dev->udev, urb->transfer_buffer_length, urb->transfer_buffer, urb->transfer_dma);
#endif
		usb_free_urb(urb);	
////
}
...
static int fermi_reset_resume(struct usb_interface *intf)
{
	struct usb_fermi *dev = usb_get_intfdata(intf);
	fermi_start_bulk_reads(dev);
	return 0;
...
static struct usb_driver fermi_driver = {
	...
	.resume =	fermi_resume,
	.reset_resume =	fermi_reset_resume,
